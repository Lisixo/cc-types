/// <reference path="./types/globals/_G.d.ts" />
/// <reference path="./types/globals/colors.d.ts" />
/// <reference path="./types/globals/colours.d.ts" />
/// <reference path="./types/globals/command.d.ts" />
/// <reference path="./types/globals/fs.d.ts" />
/// <reference path="./types/globals/gps.d.ts" />
/// <reference path="./types/globals/http.d.ts" />
/// <reference path="./types/globals/os.d.ts" />
/// <reference path="./types/globals/paintutils.d.ts" />
/// <reference path="./types/globals/peripheral.d.ts" />
/// <reference path="./types/globals/rednet.d.ts" />
/// <reference path="./types/globals/term.d.ts" />
/// <reference path="./types/globals/textutils.d.ts" />
/// <reference path="./types/globals/turtle.d.ts" />

/// <reference path="./types/modules/cc.audio.dfpwm.d.ts" />
/// <reference path="./types/modules/cc.completion.d.ts" />
/// <reference path="./types/modules/cc.expect.d.ts" />
/// <reference path="./types/modules/cc.image.nft.d.ts" />

/// <reference path="./types/peripherals/command.d.ts" />
/// <reference path="./types/peripherals/computer.d.ts" />
/// <reference path="./types/peripherals/drive.d.ts" />
/// <reference path="./types/peripherals/modem.d.ts" />
/// <reference path="./types/peripherals/monitor.d.ts" />
/// <reference path="./types/peripherals/printer.d.ts" />
/// <reference path="./types/peripherals/speaker.d.ts" />

/// <reference path="./types/peripherals/generic/energyStorage.d.ts" />
/// <reference path="./types/peripherals/generic/fluidStorage.d.ts" />
/// <reference path="./types/peripherals/generic/inventory.d.ts" />

/// <reference path="./types/events.d.ts" />
